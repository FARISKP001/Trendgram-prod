...
const ChatInput = ({
  ...
  // Show input for chatting, connecting (disabled), or disconnected states
  if (!(chatState === 'chatting' || chatState === 'connecting' || chatState === 'disconnected')) return null;
  ...
  return (
      ...
      <div className="flex items-end gap-2">
        ...
        <div className={`flex-1 rounded-2xl border ${theme === 'dark' ? 'border-gray-600' : 'border-black'} bg-white dark:bg-gray-700 shadow-md px-3 py-1`}>
          <textarea
            ...
            onKeyDown={onKeyDown}
            rows={1}
            placeholder={chatState === 'connecting' ? 'Waiting for partner to connect...' : 'Type a message...'}
            required
            autoFocus
            disabled={chatState === 'disconnected' || chatState === 'connecting'}
            className={`
    ...
    `}
        ...
        <button
          type="submit"
          disabled={!input.trim() || !!inputError || chatState === 'disconnected' || chatState === 'connecting'}
          className={`
